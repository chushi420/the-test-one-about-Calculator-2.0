...
//                Toast.makeText(this,"即将退出",Toast.LENGTH_LONG).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button2to10=(Button)findViewById(R.id.button2to10);
        ...
        buttonc.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {in.setHint("0");in.setText("");out.setText("");
            }
        });
        ...
        buttonback.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {if(in.getText().toString().isEmpty()) {return;}
        ...
        buttonequal.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                double num=0;//存放取数
                char op1,op2,opresult;//俩操作符 和一个比较结果
                double r=0;
                double num1,num2;
...
//                head=last+1;//改变头 确定下一个范围的头
    ...
    class runurllength implements Runnable {//重写run 实现耗时运算
        //继承Thread和实现Runnablle区别：java单继承机制，使用继承会有限制，另外，使用继承不可以实现资源共享
    ...
    class runurltime implements Runnable {//重写run 实现耗时运算
         ...
         *
         * Runnable接口
         * (1)这个接口应该被那些有实例并且要执行线程的类实现。
         * (2)这个类必须定义一个无参的方法叫做run方法。
         * (3)这个接口被设计用来给对象提供一个共通的协议，这些对象在活动时可以执行代码。
         * (4)Thread类实现了Runnable接口，活动着简单的理解就是线程已经开始到现在还没有停止执行。
         * (5)此外,Runnable接口提供了一个类保持活动的方法但是他的子类却不是其子线程。
         * (6)一个实现Runnable接口的类可以在没有子线程的情况下执行run方法,比如，一个线程实例传递自己作为目标。
         * (7)在大多数情况下,如果你仅仅计划重写run方法，其他线程方法不重写的情况下，Runnable接口应该被使用.
         * @刁宇鹏
         *
         */
        @Override
